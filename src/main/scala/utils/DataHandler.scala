...
object DataHandler {
  ...
  def createDataFrame(graphs: String = "caida"): DataFrame = {

    val path =
      if (graphs == "caida") "file:///home/ozzy/IdeaProjects/multi-layered-triangles-master/src/main/resources/as-caida"
      else "file:///home/ozzy/IdeaProjects/multi-layered-triangles-master/src/main/resources/as-733"
  ...
  def processFileName(input_text: String): String = {
    input_text.split("/").takeRight(2).mkString(" ")
  }
  ...
  def processDataFrame(df: DataFrame): DataFrame = {
    df.withColumn("tmp", split(col("raw"), "\\s+"))
      ...
      .agg(first("src"), first("dest"), first("input_file"))
      .drop("first(src)", "first(dest)")
      .groupBy("graph_id", "first(input_file)")
